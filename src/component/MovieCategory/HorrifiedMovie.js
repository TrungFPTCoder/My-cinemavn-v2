...
function HorrifiedMovie({ cate }) {
    ...
    return (
        <div className="position-relative"> {/* Thêm position-relative để định vị nút */}
            {horrifiedMovies.items?.length > 0 ? (
                <>
                    <Swiper
                        ...
                        slidesPerView={6}
                        navigation={{
                            prevEl: '.horrified-movie-prev',
                            nextEl: '.horrified-movie-next',
                        }}
                        grabCursor={true}
                        autoplay={{ delay: 4000 }}
                    ...
                    >
                        {horrifiedMovies.items?.map((horrifiedMovie) => (
                            <SwiperSlide key={horrifiedMovie.id}>
                                <div className="card border-0 movie--width">
                                    <div className='img-container overflow-hidden position-relative'>
                                        <img src={horrifiedMovie.thumb_url || '/images/updating_image.png'} className="card-img hover-thumb" alt={horrifiedMovie.name} />
                    ...
                    {/* Nút custom */}
                    <div className="horrified-movie-prev position-absolute top-50 translate-middle-y" style={{ zIndex: 10, cursor: 'pointer', left: '10px', backgroundColor: 'rgba(0, 0, 0, 0.6)', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <FontAwesomeIcon icon={faChevronLeft} size="lg" color="white" />
                    </div>
                    <div className="horrified-movie-next position-absolute top-50 translate-middle-y" style={{ zIndex: 10, cursor: 'pointer', right: '10px', backgroundColor: 'rgba(0, 0, 0, 0.6)', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
