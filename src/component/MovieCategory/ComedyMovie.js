...
import { setComedyMovies } from '../Slice/ComedyMoviesSlice';
            ...
            {comedyMovies.items?.length > 0 ? (
                <>
                    <Swiper
                        ...
                        slidesPerView={6}
                        navigation={{
                            prevEl: '.comedy-movie-prev',
                            nextEl: '.comedy-movie-next',
                        }}
                        grabCursor={true}
                        autoplay={{ delay: 4000 }}
                    ...
                    >
                        {comedyMovies.items?.map((comedyMovie) => (
                            <SwiperSlide key={comedyMovie.id}>
                                <div className="card border-0 movie--width">
                                    <div className='img-container overflow-hidden position-relative'>
                                        <img src={comedyMovie.thumb_url || '/images/updating_image.png'} className="card-img hover-thumb" alt={comedyMovie.name} />
                    ...
                    {/* Nút custom */}
                    <div className="comedy-movie-prev position-absolute top-50 translate-middle-y" style={{ zIndex: 10, cursor: 'pointer', left: '10px', backgroundColor: 'rgba(0, 0, 0, 0.6)', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <FontAwesomeIcon icon={faChevronLeft} size="lg" color="white" />
                    </div>
                    <div className="comedy-movie-next position-absolute top-50 translate-middle-y" style={{ zIndex: 10, cursor: 'pointer', right: '10px', backgroundColor: 'rgba(0, 0, 0, 0.6)', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
