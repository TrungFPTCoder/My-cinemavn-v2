...
import { fetchNewMovies } from "../kkPhim-service/MovieService";
import { setNewMovies } from "./MovieStore";
import { Link } from "react-router-dom";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import { faPlayCircle } from "@fortawesome/free-regular-svg-icons";
import '../kkPhim-assest/MovieList.css';
import { Helmet } from "react-helmet";
import LoadingComponent from "./LoadingComponent";
import { fetchMovieDetails } from '../kkPhim-service/MovieService';
...
function MovieList() {
  ...
  return (
    <div className="container-fluid bg-dark text-light pb-5">
      <Helmet>
      ...
      >
        {newMovies.items.map((newMovie) => (
          <SwiperSlide key={newMovie._id}>
            <div className='img-container position-relative swiper--img mobile--header'>
              <img src={newMovie.thumb_url} className="w-100 object-fit-cover" alt={newMovie.name} />
              <div className="position-absolute mobile--view" style={{ top: '40%', left: '20px' }}>
                <div className="text-warp">
                  <p className="text-primary bg-dark opacity-75 h1 rounded p-2" style={{ width: 'fit-content' }}>{newMovie.name}</p>
                </div>
                <div className='d-flex mt-3'>
                  ...
                  <div className='border border-light border-1 mx-1 p-1 px-2 rounded' style={{ backgroundColor: '#343232', opacity: '0.7' }}>
                    {movieDetails.type === 'single' ? 'Phim lẻ' : 'Phim bộ'}
                  </div>
                  <div className='border border-light border-1 mx-1 p-1 px-2 rounded' style={{ backgroundColor: '#343232', opacity: '0.7' }}>
                    {movieDetails.episode_current}
                  </div>
                  <div className='border border-light border-1 mx-1 p-1 px-2 rounded' style={{ backgroundColor: '#343232', opacity: '0.7' }}>
                    {movieDetails.time === '' ? 'Đang cập nhật' : movieDetails.time}
                  </div>
                </div>
                <div className='d-flex mt-2'>
                  {movieDetails.category?.map((item) => (
                ...
                <div className="mt-2 des-movie">
                  <p className='text-light bg-dark opacity-75 rounded p-1'>{movieDetails.content}</p>
                </div>
              </div>
